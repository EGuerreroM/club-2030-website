...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Viewport ref={ref} className={viewportStyles({ className })} {...props} />
));
...
const toastVariants = tv({
  ...
  variants: {
    variant: {
      ...
      success: 'group border-green-600 bg-green-300 text-green-800',
      error: ' group border-red-600 bg-red-400 text-white',
    },
  },
  defaultVariants: {
    variant: 'default',
...
>(({ className, variant, ...props }, ref) => {
  return <ToastPrimitives.Root ref={ref} className={toastVariants({ variant, className })} {...props} />;
});
...
>(({ className, ...props }, ref) => <ToastPrimitives.Action className={actionStyles({ className })} ref={ref} {...props} />);
ToastAction.displayName = ToastPrimitives.Action.displayName;

const closeStyle = tv({
  base: 'absolute right-2 top-2 rounded-md p-1 text-foreground/50 opacity-0 transition-opacity hover:text-foreground focus:opacity-100 focus:outline-none focus:ring-2 group-hover:opacity-100 group-[.destructive]:text-red-300 group-[.destructive]:hover:text-red-50 group-[.destructive]:focus:ring-red-400 group-[.destructive]:focus:ring-offset-red-600',
});
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Close ref={ref} className={closeStyle({ className })} toast-close="" {...props}>
    <X className="h-4 w-4" />
...
>(({ className, ...props }, ref) => <ToastPrimitives.Title ref={ref} className={titleStyle({ className })} {...props} />);
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Description ref={ref} className={description({ className })} {...props} />
));
